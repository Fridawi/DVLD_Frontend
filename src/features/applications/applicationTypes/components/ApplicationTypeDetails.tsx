...
export default function ApplicationTypeDetails(): JSX.Element {
  const { applicationTypeID } = useParams<{ applicationTypeID: string }>();
  const navigate = useNavigate();
  const numericID = Number(applicationTypeID);
          ...
          <div className="flex items-center gap-4">
            {user?.role === "Admin" && (
              <button
                onClick={() =>
                  navigate(
                    `/applications/types/edit/${appType.applicationTypeID}`,
                  )
                }
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-[10px] font-black text-slate-400 uppercase leading-none mb-1">
                  Type ID: #{appType.applicationTypeID}
                </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-8 gap-x-12">
            ...
            <div className="sm:col-span-2 lg:col-span-3 pt-2">
              <div className="flex items-start gap-3 p-4 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                ...
                <div className="flex flex-col">
                  ...
                  <p className="text-xs text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">
                    This type defines official fees and naming across local and
                    international transactions. Any changes made here will
                    reflect immediately across all new and pending applications
                    in the DVLD system.
                  </p>
                </div>
              </div>
