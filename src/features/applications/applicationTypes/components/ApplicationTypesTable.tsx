...
export default function ApplicationTypesTable({ applicationTypes }: Props) {
          ...
          {applicationTypes.map((applicationType) => (
            ...
            >
              ...
              <td className="px-6 py-4 text-gray-900 dark:text-white">
                {applicationType.fees.toLocaleString("en-US", {
                  style: "currency",
                  currency: "USD",
                })}
              </td>
                        ...
                        <MenuItem>
                          {({ active }) => (
                            <button
                              onClick={() =>
                                navigate(
                                  `/applications/types/${applicationType.applicationTypeID}`,
                                )
                              }
                              className={`${
                                active
                          ...
                          <MenuItem>
                            {({ active }) => (
                              <button
                                onClick={() =>
                                  navigate(
                                    `/applications/types/edit/${applicationType.applicationTypeID}`,
                                  )
                                }
                                className={`${
                                  active
